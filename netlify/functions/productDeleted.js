...
const { getSellerDeletedTemplate, getAdminDeletedTemplate } = require('./productDeletedTemplate.js');
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (data.event !== 'SELLER_PRODUCT_DELETED') {
        ...
        if (data.sellerEmail && data.sellerEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: data.sellerEmail,
                subject: `Product Deleted Confirmation - ${data.productName}`,
                html: getSellerDeletedTemplate(data)
            });
        }
        ...
        await transporter.sendMail({
            from: smtpFrom,
            to: DEVELOPER_EMAIL,
            subject: `Product Deleted: ${data.productName} (${data.sellerId})`,
            html: getAdminDeletedTemplate(data)
        });

        return {
            statusCode: 200,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ success: true, message: "Product deleted notification emails dispatched successfully." })
        };

    } catch (error) {
        console.error("Product Deleted Email Error:", error);
