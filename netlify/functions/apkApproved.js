...
const { getApkApprovedTemplate } = require('./apkApprovedTemplate.js');
    ...
    try {
        ...
        if (data.sellerEmail && data.sellerEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: data.sellerEmail,
                subject: `Your Android Application is Now Available`,
                html: getApkApprovedTemplate(data)
            });
        }
        ...
        return {
            ...
            body: JSON.stringify({ success: true, message: "APK approval notification email dispatched successfully." })
        };

    } catch (error) {
        console.error("APK Approved Email Error:", error);
