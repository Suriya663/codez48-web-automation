...
const { getSellerUpdatedTemplate, getAdminUpdatedTemplate } = require('./productUpdatedTemplate.js');
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (data.event !== 'SELLER_PRODUCT_UPDATED') {
        ...
        if (data.sellerEmail && data.sellerEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: data.sellerEmail,
                subject: `Product Updated Confirmation - ${data.productName}`,
                html: getSellerUpdatedTemplate(data)
            });
        }
        ...
        await transporter.sendMail({
            from: smtpFrom,
            to: DEVELOPER_EMAIL,
            subject: `Product Updated: ${data.productName} (${data.sellerId})`,
            html: getAdminUpdatedTemplate(data)
        });

        return {
            statusCode: 200,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ success: true, message: "Product updated notification emails dispatched successfully." })
        };

    } catch (error) {
        console.error("Product Updated Email Error:", error);
