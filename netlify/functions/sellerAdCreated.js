...
const { getSellerAdCreatedTemplate, getAdminAdCreatedTemplate } = require('./sellerAdCreatedTemplate.js');
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (data.event !== 'SELLER_AD_CREATED') {
        ...
        if (data.sellerEmail && data.sellerEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: data.sellerEmail,
                subject: `Advertisement Successfully Created - ${data.title}`,
                html: getSellerAdCreatedTemplate(data)
            });
        }
        ...
        await transporter.sendMail({
            from: smtpFrom,
            to: DEVELOPER_EMAIL,
            subject: `New Seller Advertisement Created (${data.sellerId})`,
            html: getAdminAdCreatedTemplate(data)
        });

        return {
            statusCode: 200,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ success: true, message: "Seller ad created notification emails dispatched successfully." })
        };

    } catch (error) {
        console.error("Seller Ad Created Email Error:", error);
