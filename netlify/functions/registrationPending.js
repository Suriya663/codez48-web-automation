...
const { getUserEmailTemplate, getAdminEmailTemplate } = require('./registrationPendingTemplate.js');
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        // Ensure this is exactly the requested event
        if (data.event !== 'PROFILE_REGISTRATION_PAYMENT_PENDING') {
        ...
        // Email A: To the registered user
        if (targetUserEmail && targetUserEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: targetUserEmail,
                subject: `CODEZ48 Registration Received - Payment Pending`,
        ...
        await transporter.sendMail({
            from: smtpFrom,
            to: DEVELOPER_EMAIL,
            subject: `New Registration – Payment Pending (${data.sellerId})`,
        ...
        return {
            ...
            body: JSON.stringify({
                success: true,
                message: "Profile registration (payment pending) emails dispatched."
            })
        };
