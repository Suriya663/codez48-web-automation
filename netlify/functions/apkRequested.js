...
const { getSellerApkReqTemplate, getAdminApkReqTemplate } = require('./apkRequestedTemplate.js');
    ...
    try {
        ...
        if (data.sellerEmail && data.sellerEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: data.sellerEmail,
                subject: `Android Application / APK Request Submitted`,
                html: getSellerApkReqTemplate(data)
            });
        }
        ...
        await transporter.sendMail({
            from: smtpFrom,
            to: DEVELOPER_EMAIL,
            subject: `APK/Android Application Requested (${data.sellerId})`,
            html: getAdminApkReqTemplate(data)
        });

        return {
            statusCode: 200,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ success: true, message: "APK requested notification emails dispatched successfully." })
        };

    } catch (error) {
        console.error("APK Requested Email Error:", error);
