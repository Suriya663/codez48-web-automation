...
const { getRejectedTemplate } = require('./collabRejectedTemplate.js');
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (data.event !== 'COLLABORATION_REJECTED') {
        ...
        if (data.senderEmail && data.senderEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: data.senderEmail,
                subject: `Collaboration Request Declined by ${data.receiverBrand || 'Partner'}`,
        ...
        return {
            ...
            body: JSON.stringify({ success: true, message: "Collaboration rejection email sent." })
        };
    } catch (error) {
        console.error("Collab Rejected Email Error:", error);
