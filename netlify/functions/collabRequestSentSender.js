...
const { getSenderTemplate } = require('./collabRequestSentSenderTemplate.js');
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (data.event !== 'COLLAB_REQUEST_SENT_SENDER') {
        ...
        if (data.senderEmail && data.senderEmail.includes('@')) {
            await transporter.sendMail({
                from: smtpFrom,
                to: data.senderEmail,
                subject: `Collaboration Request Sent Successfully to ${data.receiverBrand || 'Partner'}`,
                html: getSenderTemplate(data)
            });
        }
        ...
        return {
            ...
            body: JSON.stringify({ success: true, message: "Sender confirmation email sent." })
        };
    } catch (error) {
        console.error("Collab Sender Email Error:", error);
